...
module Impl = {
  ...
  let transparentizeAlphaValue = (alpha: Percent.t, percentage: Percent.t) =>
    Percent.make(
      floatInRange(
        (Percent.asFloat(alpha) *. 100.0 -. Percent.asFloat(percentage) *. 100.0) /. 100.0,
        0.0,
        1.0,
      ),
    )
  ...
  let transparentizeRGBA = (rgba: RGBA.t, percentage: Percent.t) => {
    ...
    RGBA.make(~red=RGBA.red(rgba), ~green=RGBA.green(rgba), ~blue=RGBA.blue(rgba), ~alpha)
  }
...
let transparentize = (color: color, percentage: Percent.t): color =>
  ...
  | HEX(hex) =>
    ...
    ->RGBA
  | RGB(rgb) => rgb->convertRGBtoRGBA->Impl.transparentizeRGBA(percentage)->RGBA
  | RGBA(rgba) => rgba->Impl.transparentizeRGBA(percentage)->RGBA
  | HSL(hsl) => hsl->convertHSLtoHSLA->Impl.transparentizeHSLA(percentage)->HSLA
