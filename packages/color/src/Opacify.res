...
module Impl = {
  ...
  let opacifyAlphaValue = (alpha: Percent.t, amount: Percent.t) =>
    Percent.make(
      floatInRange(
        (alpha->Percent.asFloat *. 100.0 +. amount->Percent.asFloat *. 100.0) /. 100.0,
        0.0,
        1.0,
      ),
    )
  ...
  let opacifyRGBA = (rgba: RGBA.t, amount: Percent.t) => {
    ...
    RGBA.make(~red=rgba->RGBA.red, ~green=rgba->RGBA.green, ~blue=rgba->RGBA.blue, ~alpha)
  }
...
let opacify = (color: color, amount: Percent.t) =>
  switch color {
  | HEX(hex) => hex->convertHEXtoRGB->convertRGBtoRGBA->Impl.opacifyRGBA(amount)->RGBA
