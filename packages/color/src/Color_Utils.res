...
let convertRGBtoHEX = (rgb: RGB.t): HEX.t => {
  ...
  let strGreen = green->Int8.asInt->Js.Int.toStringWithRadix(~radix=16)->padStart
  let strBlue = blue->Int8.asInt->Js.Int.toStringWithRadix(~radix=16)->padStart
  HEX.make(strRed ++ (strGreen ++ strBlue))
}

let convertRGBAtoHSLA = (rgba: RGBA.t): HSLA.t => {
  let (red, green, blue, alpha) = (
    Int8.asInt(rgba->RGBA.red),
    Int8.asInt(rgba->RGBA.green),
    Int8.asInt(rgba->RGBA.blue),
    Percent.asFloat(rgba->RGBA.alpha),
  )
...
let convertHSLAtoRGBA = (hsla: HSLA.t): RGBA.t => {
  let (h, s, l, a) = (
    Degree.asFloat(hsla->HSLA.hue),
    Percent.asFloat(hsla->HSLA.saturation),
    Percent.asFloat(hsla->HSLA.lightness),
    Percent.asFloat(hsla->HSLA.alpha),
  )
...
let convertRGBAtoRGB = (rgba: RGBA.t): RGB.t =>
  RGB.make(~red=rgba->RGBA.red, ~green=rgba->RGBA.green, ~blue=rgba->RGBA.blue)
...
let convertHSLAtoHSL = (hsla: HSLA.t): HSL.t =>
  HSL.make(~hue=hsla->HSLA.hue, ~saturation=hsla->HSLA.saturation, ~lightness=hsla->HSLA.lightness)
...
let getLuminance = (color: color): float => {
  ...
  let (r, g, b) = (
    Int8.toFloat(rgb->RGB.red),
    Int8.toFloat(rgb->RGB.green),
    Int8.toFloat(rgb->RGB.blue),
  )
