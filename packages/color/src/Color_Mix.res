...
let mix = (color: color, baseColor: color, weight: Percent.t): color => {
  ...
  @ocaml.doc(" The formula is copied from the original Sass implementation:
  ...
  let normalizedWeight =
    Percent.asFloat(weight) *. 2.0 -. 1.0
  let alphaDistance =
    Percent.asFloat(RGBA.alpha(colorRgba)) -. Percent.asFloat(RGBA.alpha(baseColorRgba))
  ...
  let red = int_of_float(
    floor(
      Int8.toFloat(RGBA.red(colorRgba)) *. weight1 +.
        Int8.toFloat(RGBA.red(baseColorRgba)) *. weight2,
    ),
  )
  let green = int_of_float(
    floor(
      Int8.toFloat(RGBA.green(colorRgba)) *. weight1 +.
        Int8.toFloat(RGBA.green(baseColorRgba)) *. weight2,
    ),
  )
  let blue = int_of_float(
    floor(
      Int8.toFloat(RGBA.blue(colorRgba)) *. weight1 +.
        Int8.toFloat(RGBA.blue(baseColorRgba)) *. weight2,
    ),
  )
  let alpha =
    Percent.asFloat(RGBA.alpha(colorRgba)) *. Percent.asFloat(weight) +.
      Percent.asFloat(RGBA.alpha(baseColorRgba)) *. (1.0 -. Percent.asFloat(weight))

  switch (color, baseColor) {
  | (RGB(_) | HEX(_) | HSL(_), RGB(_)) => RGB(RGB.fromPrimitives(red, green, blue))
  | (RGB(_) | HEX(_) | HSL(_), HEX(_)) => HEX(convertRGBtoHEX(RGB.fromPrimitives(red, green, blue)))
  | (RGB(_) | HEX(_) | HSL(_), HSL(_)) => HSL(convertRGBtoHSL(RGB.fromPrimitives(red, green, blue)))
  | (_, RGBA(_)) | (RGBA(_), RGB(_) | HEX(_) | HSL(_)) =>
    RGBA(RGBA.fromPrimitives(red, green, blue, alpha))
  | (_, HSLA(_)) | (HSLA(_), RGB(_) | HEX(_) | HSL(_)) =>
