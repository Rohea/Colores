...
module Impl = {
  let tint = (color: color, amount: Percent.t): color =>
    Color_Mix.mix(RGB(RGB.fromPrimitives(255, 255, 255)), color, amount)

  let tintRGBA = (rgba: RGBA.t, amount: Percent.t): color => {
    let alpha = RGBA.alpha(rgba)
    let rgb = Color_Utils.convertRGBAtoRGB(rgba)

    let tintdRgb = tint(RGB(rgb), amount)
    Color_Transparentize.transparentize(tintdRgb, alpha)
  }

  let tintHSLA = (hsla: HSLA.t, amount: Percent.t): color => {
    let alpha = HSLA.alpha(hsla)
    let hsl = Color_Utils.convertHSLAtoHSL(hsla)

    let tintedHsl = tint(HSL(hsl), amount)
    Color_Transparentize.transparentize(tintedHsl, alpha)
  }
}
