...
let darken = (color: color, amount: Percent.t) =>
  ...
  | RGB(rgb) =>
    ...
    ->convertRGBAtoRGB
    ->RGB
  | RGBA(rgba) => rgba->convertRGBAtoHSLA->Impl.darkenHSLA(amount)->convertHSLAtoRGBA->RGBA
  | HSL(hsl) => hsl->convertHSLtoHSLA->Impl.darkenHSLA(amount)->convertHSLAtoHSL->HSL
