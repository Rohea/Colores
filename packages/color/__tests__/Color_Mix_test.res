...
describe("Color_Mix.mix", () => {
  let expectedColor = RGBA(RGBA.fromPrimitives(0, 128, 255, 0.5))

  test("if weight 0, return baseColor", () =>
    expect(
      mix(RGBA(RGBA.fromPrimitives(0, 128, 255, 1.0)), expectedColor, Percent.make(0.0)),
  ...
  test("should mix #0080FF with #000000 at weight 1.0 and return #0080FF", () =>
    expect(mix(HEX(HEX.make("0080FF")), HEX(HEX.make("000000")), Percent.make(1.0)))->toEqual(
      HEX(HEX.make("0080FF")),
    )
  )

  test("should mix #0080FF with #000000 at weight 0.5 and return #00407F", () =>
    expect(mix(HEX(HEX.make("0080FF")), HEX(HEX.make("000000")), Percent.make(0.5)))->toEqual(
      HEX(HEX.make("00407F")),
    )
  )

  test("should mix rgb(0, 0, 0) with rgb(0, 128, 255) at weight 1.0 and return rgb(0, 0, 0)", () =>
    expect(
      mix(
        RGB(RGB.fromPrimitives(0, 0, 0)),
        RGB(RGB.fromPrimitives(0, 128, 255)),
  ...
  test(
    ...
    () =>
      ...
      )->toEqual(HSLA(HSLA.fromPrimitives(209.52380952380952, 1.0, 0.12352941176470589, 0.75))),
  )
