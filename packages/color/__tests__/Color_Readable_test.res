...
describe("Color_Readable", () => {
  ...
  test("should return onLight color for rgb(128, 128, 128) background color", () =>
  ...
  test(
    ...
    () => expect(readable(bgDark, ()))->toBe(Color_Readable.defaultDarkReturnColor),
  )

  test(
    "should return defaultOnLight color because of light background color and no other specified",
    () => expect(readable(bgLight, ()))->toBe(Color_Readable.defaultLightReturnColor),
  )
})
