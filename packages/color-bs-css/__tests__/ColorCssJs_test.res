...
describe("PolishedCss_Color.Utils.cssAngleToDegree", () => {
  ...
  test("0 grad to 0 deg", () => expect(Utils.cssAngleToDegree(#grad(0.0)))->toBe(Degree.make(0.0)))

  test("400 grad to 360 deg", () =>
    expect(Utils.cssAngleToDegree(#grad(400.0)))->toBe(Degree.make(360.0))
  )
  ...
  test("0 turn to 0 deg", () => expect(Utils.cssAngleToDegree(#turn(0.0)))->toBe(Degree.make(0.0)))

  test("45 deg to 45 deg", () =>
    expect(Utils.cssAngleToDegree(#deg(45.0)))->toBe(Degree.make(45.0))
  )

  test("0 deg to 0 deg", () => expect(Utils.cssAngleToDegree(#deg(0.0)))->toBe(Degree.make(0.0)))

  test("360 deg to 360 deg", () =>
    expect(Utils.cssAngleToDegree(#deg(360.0)))->toBe(Degree.make(360.0))
  )

  test("3.141592654 rad to 179.9087 deg", () =>
    expect(Utils.cssAngleToDegree(#rad(3.141592654))->Degree.asFloat)->toBeSoCloseTo(
      Degree.make(180.0)->Degree.asFloat,
      ~digits=4,
    )
  )

  test("0.0 rad to 0 deg", () => expect(Utils.cssAngleToDegree(#rad(0.0)))->toBe(Degree.make(0.0)))

  test("6.283185415 rad to 360 deg", () =>
    expect(Utils.cssAngleToDegree(#rad(6.283185415)))->toBe(Degree.make(360.0))
  )
})
...
describe("PolishedCss_Color.Utils.cssToColor", () => {
  ...
  test("bs-css rgba to polished rgba", () =>
    expect(Utils.cssToColor(#rgba(0, 128, 255, #num(0.5))))->toEqual(
      Some(RGBA(RGBA.fromPrimitives(0, 128, 255, 0.5))),
    )
  )

  test("bs-css hex to polished hex", () =>
    expect(Utils.cssToColor(#hex("0080FF")))->toEqual(Some(HEX(HEX.make("0080FF"))))
  )

  test("bs-css hsl to polished hsl", () =>
    expect(Utils.cssToColor(#hsl(#deg(210.0), #percent(100.0), #percent(50.0))))->toEqual(
      Some(HSL(HSL.fromPrimitives(210.0, 1.0, 0.5))),
    )
  )

  test("bs-css hsla to polished hsla", () =>
    expect(
      Utils.cssToColor(#hsla(#deg(210.0), #percent(100.0), #percent(50.0), #num(0.5))),
    )->toEqual(Some(HSLA(HSLA.fromPrimitives(210.0, 1.0, 0.5, 0.5))))
  )

  test("bs-css transparent to polished rgba", () =>
    expect(Utils.cssToColor(#transparent))->toEqual(Some(RGBA(RGBA.fromPrimitives(0, 0, 0, 0.0))))
  )
})
...
describe("PolishedCss_Color.Utils.colorToCss", () => {
  ...
  test("polished rgba to bs-css rgba", () =>
    expect(Utils.colorToCss(RGBA(RGBA.fromPrimitives(0, 128, 255, 0.5))))->toEqual(
      #rgba(0, 128, 255, #percent(50.0)),
    )
  )

  test("polished hex to bs-css hex", () =>
    expect(Utils.colorToCss(HEX(HEX.make("0080ff"))))->toEqual(#hex("0080ff"))
  )

  test("polished hex to bs-css hex with decapitalization", () =>
    expect(Utils.colorToCss(HEX(HEX.make("AABBFF"))))->toEqual(#hex("aabbff"))
  )

  test("polished hsl to bs-css hsl", () =>
    expect(Utils.colorToCss(HSL(HSL.fromPrimitives(210.0, 1.0, 0.5))))->toEqual(
      #hsl(#deg(210.0), #percent(100.0), #percent(50.0)),
    )
  )

  test("polished hsla to bs-css hsla", () =>
    expect(Utils.colorToCss(HSLA(HSLA.fromPrimitives(210.0, 1.0, 0.5, 0.5))))->toEqual(
      #hsla(#deg(210.0), #percent(100.0), #percent(50.0), #percent(50.0)),
    )
  )
})
...
describe("PolishedCss_Color.transparentize", () => {
  ...
  test("transparentize rgba", () =>
    expect(transparentize(#rgba(0, 128, 255, #num(0.7)), 0.5))->toEqual(
      #rgba(0, 128, 255, #percent(20.0)),
    )
  )
})
...
describe("PolishedCss_Color.readable", () => {
  ...
  test("should return onDark color because of dark background color", () =>
    expect(readable(#rgb(20, 20, 20), ~onLight, ~onDark, ()))->toEqual(onDark)
  )

  test("should return onLight color because of light background color", () =>
    expect(readable(#rgb(200, 200, 200), ~onLight, ~onDark, ()))->toEqual(onLight)
  )
})
...
describe("PolishedCss_Color.mix", () => {
  ...
  test("mix two rgba colors", () =>
    expect(
      mix(#rgba(0, 255, 128, #percent(50.0)), #rgba(128, 0, 255, #percent(50.0)), 0.5),
...
describe("PolishedCss_Color.setAlpha", () => {
  ...
  test("setAlpha of rgba", () =>
    expect(setAlpha(#rgba(0, 128, 255, #percent(70.0)), 0.5))->toEqual(
      #rgba(0, 128, 255, #percent(50.0)),
    )
  )
})
