...
module Impl = {
  let lightenHSLA = (hsla: HSLA.t, amount: Percent.t) => {
    ...
    let newLightness = (lightnessF +. amount->Percent.asFloat)->floatInRange(0.0, 1.0)
    HSLA.make(
      ~hue=hsla->HSLA.hue,
...
let lighten = (color: color, amount: Percent.t) =>
  ...
  | RGB(rgb) =>
    ...
    ->convertRGBAtoRGB
    ->RGB
  | RGBA(rgba) => rgba->convertRGBAtoHSLA->Impl.lightenHSLA(amount)->convertHSLAtoRGBA->RGBA
  | HSL(hsl) => hsl->convertHSLtoHSLA->Impl.lightenHSLA(amount)->convertHSLAtoHSL->HSL
